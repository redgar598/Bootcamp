...
# MAP File 
### There are many online resources for how these files are shaped and what values need to be in them.
```{r}
...
PAWS_snps_filtered_map_unique<-unique(PAWS_snps_filtered_map) # 532861 SNPs
...
PAWS_snps_filtered_map_unique2<-PAWS_snps_filtered_map_unique2[,c("Chr.y","SNP_Name","cM","MapInfo")]

write.table(as.matrix(PAWS_snps_filtered_map_unique2), "PAWS_snps_filtered_MAP.txt", sep="\t", row.names = F,col.names = F,quote=FALSE)
```
...
## Save the PED file
save(PAWS_snps_filtered_ped_melt_dcast, file="PAWS_snps_filtered_ped_melt_dcast.RData")
