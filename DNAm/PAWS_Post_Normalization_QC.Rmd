...
### Confirm Twin IDS and add to meat data
```{r}
load("PAWS_Snps_betas.RData")
...
# calculate Blood proportion
```{r}
# add age matched whole blood to paws
load("GEO_PAWS_aged_matched_whole_blood.RData")
Paws_meta_min<-meta[,c(1,8)]
Paws_meta_min$series_id<-"PAWS"
Paws_meta_min$tissue<-"Buccal"
colnames(Paws_meta_min)<-c("gsm","age_fix","series_id","tissue")
Paws_meta_min<-Paws_meta_min[,c(3,1,2,4)]
GEO_betas<-GEO_betas[which(rownames(GEO_betas)%in%rownames(PAWS_Beta)),]
GEO_PAWS_meta<-rbind(Paws_meta_min, PAWS_GEO_meta)
GEO_PAWS_beta<-cbind(PAWS_Beta, GEO_betas)
GEO_PAWS_meta<-GEO_PAWS_meta[match(colnames(GEO_PAWS_beta), GEO_PAWS_meta$gsm),]

#Check Age matching
ggplot(GEO_PAWS_meta, aes(age_fix, fill=tissue))+geom_histogram(color="black")+theme_bw()+
  xlab("Age")
...
ggplot(Loadings_geo,aes(Comp.1,Comp.2, color=GEO_PAWS_meta$series_id))+geom_point(shape=19,size=3 )+theme_bw()+
  scale_color_discrete(name="Dataset")
ggplot(Loadings_geo,aes(Comp.2,Comp.3, color=GEO_PAWS_meta$series_id))+geom_point(shape=19,size=3 )+theme_bw()

Blood_Proportion<-data.frame(gsm=GEO_PAWS_meta$gsm[order(Loadings_geo$Comp.2)], 
                             Blood_proportion_rank=1:nrow(GEO_PAWS_meta))

GEO_PAWS_meta_blood<-merge(GEO_PAWS_meta, Blood_Proportion, by="gsm")
GEO_PAWS_meta_blood<-GEO_PAWS_meta_blood[match(colnames(GEO_PAWS_beta), GEO_PAWS_meta_blood$gsm),]


#just PAWS
meta_blood_pro<-merge(meta,GEO_PAWS_meta_blood[,c(1,5)], by.x="Meth_ID", by.y="gsm")
...
ggplot(Loadings,aes(Comp.1,Comp.2, color=meta_blood_pro$Genetic_cluster))+geom_point(shape=19,size=3 )+theme_bw()
ggplot(Loadings,aes(Comp.2,Comp.3, color=meta_blood_pro$Genetic_cluster))+geom_point(shape=19,size=3 )+theme_bw()

ggplot(Loadings,aes(Comp.1,Comp.2, color=meta_blood_pro$Blood_proportion_rank))+
  geom_point(shape=19,size=3 )+theme_bw()+scale_color_continuous(name="Proportion of Blood")
ggplot(Loadings,aes(Comp.2,Comp.3, color=meta_blood_pro$Blood_proportion_rank))+
  geom_point(shape=19,size=3 )+theme_bw()

ggplot(Loadings,aes(Comp.1,Comp.2, color=meta_blood_pro$Age_Genetic_Collection))+
  geom_point(shape=19,size=3 )+theme_bw()
ggplot(Loadings,aes(Comp.2,Comp.3, color=meta_blood_pro$Age_Genetic_Collection))+
  geom_point(shape=19,size=3 )+theme_bw()

ggplot(Loadings[which(meta_blood_pro$CF1rungE<600),],aes(Comp.1,Comp.2, color=meta_blood_pro$CF1rungE[which(meta_blood_pro$CF1rungE<600)]))+geom_point(shape=19,size=3 )+theme_bw()
ggplot(Loadings,aes(Comp.2,Comp.3, color=meta_blood_pro$Blood_proportion_rank))+geom_point(shape=19,size=3 )+theme_bw()

```

##calculate Tcell proportion (GEO files from Documents/Side/TCells_immunomagnetic) 92 Tcell samples
```{r}
...
#load("GSE34639.RData") ## the age 0 and 1 year behave weird
#GSE34639<-GEO[[1]]
#GSE34639<-GSE34639[which(rownames(GSE34639)%in%rownames(PAWS_Beta)),]
...
summary(aov(Loadings_geo$Comp.2~ GEO_PAWS_meta$Genetic_cluster))
      ...
      #plot the correlation for continious
      GEO_PAWS_meta$Blood_proportion<-Loadings_geo$Comp.2
      PAWS_meta_withblood<-subset(GEO_PAWS_meta, series_id=="Buccal (PAWS)")
      PAWS_meta_withblood_melt<-melt(PAWS_meta_withblood, id=c("series_id","gsm","tissue","Blood_proportion","Genetic_cluster"))
      
      ggplot(PAWS_meta_withblood_melt, aes(value, Blood_proportion))+
        geom_point(aes(color=as.factor(Genetic_cluster)),shape=19)+theme_bw()+facet_wrap(~variable, scale="free_x")+
        stat_smooth(method="lm", se=F)+scale_color_manual(values=colorRampPalette(brewer.pal(11,"RdYlGn")[1:8])(4))
      
      ## plot boxplot for catagorical
      PAWS_meta_withblood_allgen<-PAWS_meta_withblood[which(!(is.na(PAWS_meta_withblood$Genetic_cluster))),]
      
      ggplot(PAWS_meta_withblood_allgen, aes(as.factor(Genetic_cluster), Blood_proportion, fill=as.factor(Genetic_cluster)))+
        geom_boxplot()+theme_bw()+
        scale_fill_manual(values=colorRampPalette(brewer.pal(11,"RdYlGn")[1:8])(4))
                               
######################################## Just Tcells for sanity
GEO_PAWS_metat<-rbind(Paws_meta, meta_tcell)
GEO_PAWS_betat<-cbind(PAWS_Beta, GEO_betas_tcell)
PCA_full_geot<-princomp(na.omit(GEO_PAWS_betat))
Loadings_geot<-as.data.frame(unclass(PCA_full_geot$loadings))

ggplot(Loadings_geot,aes(Comp.1,Comp.2, color=GEO_PAWS_metat$series_id, alpha=GEO_PAWS_metat$deinc2dep))+
  geom_point(shape=19,size=4)+
  theme_bw()+
  scale_color_manual(values=c("#2166ac","#4d9221","#b8e186","#cb181d","#fb6a4a"),name="Dataset")
```
...
ord <- c(1,2,3,4,5,6,12,13,14,15,16,17,18,19,7,8,9,10,11)
      ...
      meta_cat<-c(6,11,20:24, 26:35,53:56, 65, 66,73:83, 87:106)  # input column numbers in meta that contain categorical variables
      #doesnt like the DECHETH_AI etc variables
      meta_cont<-c(8,25,37:51,57:64, 67:71) # input column numbers in meta that contain continuous variables

      meta_categorical<-meta[,meta_cat[1:18]]
      meta_categorical <- as.data.frame(lapply(meta_categorical , factor))
      meta_continuous<-meta[,meta_cont[1:10]]
      ord<-c(1:28)

      meta_categorical<-meta[,meta_cat[22:39]]
      meta_categorical <- as.data.frame(lapply(meta_categorical , factor))
      meta_continuous<-meta[,meta_cont[11:20]]
      ord<-c(1:28)

      meta_categorical<-meta[,meta_cat[c(19,20,21,40:54)]]
      meta_categorical <- as.data.frame(lapply(meta_categorical , factor))
      meta_continuous<-meta[,meta_cont[21:30]]
      ord<-c(1:28)
...
# COMBAT: Sentrix_ID, plate_row, dateGcollected
# LME covairates: Age_Genetic_colletion, Fam_ID, Genetic Cluster, 
```

# PC PLOTS
```{r}
ggplot(Loadings,aes(Comp.2,Comp.3, color=as.numeric(as.character(meta$Age_Genetic_Collection))))+geom_point(shape=19,size=3 )+theme_bw()+
  scale_color_continuous(name="Age")
ggplot(Loadings,aes(Comp.7,Comp.8, color=meta$Genetic_cluster))+geom_point(shape=19,size=3 )+theme_bw()
...
```{r}
meta_categorical<-meta[,c("Gender_final","Age_Genetic_Collection","DECHIETH","Genetic_cluster",
                          "Fam_ID","zrank_ord","minor_child",
                          "Sample_Well","Sample_Plate","Sentrix_ID","Sentrix_Position","plate_row","dateGcollected")]
meta_continuous<-meta[,c("BMI_Percentile","HGHEDLV2","DETOTIN2","COMPSES2","deinc2dep","FLadder","FADV_6")]
meta_cat<-meta[,c(6,11,20:24, 26:35,53:56, 65, 66,73:83, 87:106)]
meta_cont<-meta[,c(8,25,37:51,57:64, 67:71)]
meta_categorical<-cbind(meta_categorical, meta_cat)
meta_continuous<-cbind(meta_continuous, meta_cont)

meta_cor<-cbind(meta_categorical,meta_continuous)
meta_cor<-meta_cor[,-c(19,28,30,31,32,37,64)]# withoutNA
...
for (i in 2:30) wss[i] <- sum(kmeans((correlation), 
                                     centers=i)$withinss)
plot(1:30, wss, type="b", xlab="Number of Clusters",
     ylab="Within groups sum of squares")

k <- kmeans(correlation,30)
...
# Confounding of minority status and geentic cluster for inclusion in models of vairables
...
##### Meta correlation of used samples and variables
```{r}
## smaller heat
meta_continuous<-meta[,c("HGHEDLV2","deinc2dep","FADV_6")]
meta_continuous192<-meta_continuous
meta_continuous178<-meta_continuous
...
ggplot(Meta_correlation, aes(HGHEDLV2, deinc2dep))+geom_point(shape=19)+theme_bw()+stat_smooth(method="lm")
ggplot(Meta_correlation, aes(HGHEDLV2, FADV_6))+geom_point(shape=19)+theme_bw()+stat_smooth(method="lm")
ggplot(Meta_correlation, aes(FADV_6, deinc2dep))+geom_point(shape=19)+theme_bw()+stat_smooth(method="lm")


## confounded
meta_inc<-meta[which(!(is.na(meta$deinc2dep))),]
meta_inc_melt<-melt(meta_inc[,c("Meth_ID","HGHEDLV2",
                                    "deinc2dep","FADV_6")], id=c("Meth_ID","deinc2dep"))
                             
meta_inc_melt<-meta_inc_melt[complete.cases(meta_inc_melt),]
ggplot(meta_inc_melt, aes(deinc2dep,value))+
      geom_point(color="grey",shape=19)+
  facet_wrap(~variable, scales="free_y", ncol=5)+
      theme_bw()+
      xlab("deinc2dep")+  stat_smooth(method="lm")
interest<-c("Meth_ID","HGHEDLV2","deinc2dep") 

pval_lm<-data.frame(variable=interest[c(2,3)],
                 pval=sapply(c(2,3),function(x){
         coef(summary(lm(meta_age[,interest[x]]~meta_age[,"deinc2dep"])))[2,"Pr(>|t|)"]}))
pval_aov<-data.frame(variable=interest[c(3,4,6,8,10,11,13)],
                     pval=sapply(c(3,4,6,8,10,11,13),function(x){
         round(summary(aov(meta_age[,interest[x]]~meta_age[,"Age_Genetic_Collection"]))[[1]][["Pr(>F)"]][1],4)}))
...
# COMBAT: Sentrix_ID, plate_row, dateGcollected (cant correct dateGcollected, many with just one per date)
# LME covairates: Age_Genetic_colletion, Fam_ID, Genetic Cluster
          ...
          Data_cors<-function(df){
            rep_cor<-do.call(rbind,lapply(1:12, function(x) {cols<-grep(replicated_individuals[x], colnames(df))
                                                             data.frame(Comparison=replicated_individuals[x],
                                                                        Correlation=cor(df[,cols[1]],
                                                                                        df[,cols[2]],use="complete.obs"))}))
            rep_cor_20<- data.frame(Comparison=replicated_individuals[13],
                                    Correlation=c(cor(df[,5],df[,10],use="complete.obs"),
                                                  cor(df[,5],df[,17],use="complete.obs"),
                                                  cor(df[,10],df[,17],use="complete.obs")))
...
save(rep_cor, file="~/PAWS/replicate_RMSE.RData")
...
## Meta data from Nicki
```{r}
DETOTIN2<-melt(meta[, c("DETOTIN2","DETOTINC_c5")])
ggplot(DETOTIN2, aes(value,fill=variable, group=variable))+
  geom_histogram(color="black",binwidth = 1, alpha=0.5,position="identity")+theme_bw()+
  scale_fill_manual(values=c("#225ea8","#c7e9b4"))

HGHEDLV2<-melt(meta[, c("HGHEDLV2","hghedlv_3cat")])
ggplot(HGHEDLV2, aes(value,fill=variable, group=variable))+
  geom_histogram(color="black",binwidth = 1, alpha=0.5,position="identity")+theme_bw()+
  scale_fill_manual(values=c("#225ea8","#c7e9b4"))

DETOTIN2$Measure<-"Income"
HGHEDLV2$Measure<-"Education"

income_edication<-rbind(DETOTIN2, HGHEDLV2)

ggplot(income_edication, aes(value,fill=variable, group=variable))+
  geom_histogram(color="black",binwidth = 1, alpha=0.5,position="identity")+theme_bw()+
  scale_fill_manual(values=c("#3690c0","#d0d1e6","#41ab5d","#d9f0a3"))+facet_wrap(~Measure, scales="free")
